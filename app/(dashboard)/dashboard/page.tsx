...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('customers').select('id, full_name').eq('business_id', businessId).eq('card_status', 'pending'),
    supabase.from('payments').select('id, amount').eq('business_id', businessId)
      .in('payment_status', ['charged', 'succeeded']),
  ])
  ...
  const allPaymentsList  = Array.isArray(allPayments) ? allPayments : []

  const totalCustomers   = customersList.length
  const authorizedCards  = customersList.filter(c => c.card_status === 'authorized' || c.card_status === 'active').length
  const pendingCardCount = pendingCardsList.length
  const todayRevenue     = todayJobsList.reduce((s, j) => s + Number(j.amount || 0), 0)
  const totalRevenue     = allPaymentsList.reduce((s, j) => s + Number(j.amount || 0), 0)
  const totalJobs        = allPaymentsList.length

  const stats = [
    { label: "Today's Revenue", value: `$${todayRevenue.toFixed(2)}`,            icon: DollarSign, color: '#3DBF7F', bg: 'rgba(61,191,127,0.1)', change: 'Today',         href: '/dashboard/jobs' },
    { label: 'Total Revenue',   value: `$${totalRevenue.toFixed(2)}`,            icon: TrendingUp, color: '#4F8EF7', bg: 'rgba(79,142,247,0.1)', change: 'All time',      href: '/dashboard/jobs' },
    { label: 'Active Customers',value: `${authorizedCards} / ${totalCustomers}`, icon: Users,      color: '#E8B84B', bg: 'rgba(232,184,75,0.1)', change: 'Cards on file', href: '/dashboard/customers' },
    { label: 'Jobs Completed',  value: String(totalJobs),                        icon: Briefcase,  color: '#4F8EF7', bg: 'rgba(79,142,247,0.1)', change: 'Total',         href: '/dashboard/jobs' },
  ]
      ...
      {pendingCardCount > 0 && (
        <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-xl text-sm"
          ...
          <span className="font-medium" style={{ color: '#0E1117' }}>
            ...
            {' '}{pendingCardsList.map((c: any) => c.full_name).join(', ')}
          </span>
              ...
              <div className="hidden lg:block">
                {jobsList.map((job: any) => (
                  <Link key={job.id} href={`/dashboard/jobs/${job.id}`}
                    ...
                    {job.payment_status === 'succeeded' && <CheckCircle2 size={14} style={{ color: '#3DBF7F', flexShrink: 0 }} />}
                  ...
                  <Link key={job.id} href={`/dashboard/jobs/${job.id}`}
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {job.payment_status === 'succeeded' && <CheckCircle2 size={14} style={{ color: '#3DBF7F' }} />}
