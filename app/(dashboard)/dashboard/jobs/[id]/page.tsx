...
export default async function JobDetailPage({
  ...
  const isRefunded = job.payment_status === 'refunded'
  const isCharged = job.payment_status === 'charged' || job.payment_status === 'succeeded'
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      <Link href="/dashboard/jobs" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#0E1117] transition">
      ...
      <div className="rounded-2xl border border-[#DDE1EC] bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              {isRefunded && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-[rgba(107,116,144,0.1)] text-[#6B7490]">
                  Refunded
                </span>
              )}
              {isScheduled && (
          ...
          <div className="text-right shrink-0">
            <p className="text-2xl font-bold font-mono" style={{ color: isRefunded ? '#6B7490' : '#0E1117' }}>
              ${isCompleted ? Number(job.amount).toFixed(2) : serviceTotal.toFixed(2)}
            </p>
            <p className={`text-xs font-medium ${
              isRefunded ? 'text-[#6B7490]' :
              isCompleted ? 'text-[#3DBF7F]' : 'text-[#E8A020]'
            }`}>
              {isRefunded ? 'Refunded' : isCompleted ? 'Charged' : 'Pending'}
            </p>
          </div>
        </div>
        ...
        {/* Refund button — owners/managers only, card payments only, not already refunded */}
        {isOwnerOrManager && isCharged && !isRefunded && job.stripe_charge_id && (
          <div className="mt-4 pt-4 border-t border-[#DDE1EC]">
            <RefundButton
              jobId={job.id}
              stripeChargeId={job.stripe_charge_id}
      ...
      {photosWithUrls.length > 0 && (
        <div className="rounded-2xl border border-[#DDE1EC] bg-white p-6 shadow-sm">
          <h2 className="font-semibold text-[#0E1117] mb-4">Job Photos ({photosWithUrls.length})</h2>
