...
import { CheckCircle2, AlertTriangle, ArrowRight, DollarSign, TrendingUp, Clock, CheckCircle, Percent } from 'lucide-react'
    ...
    async function load() {
      try {
        ...
        const { data: member, error: memberError } = await supabase
          .from('team_members')
          .select('service_companies(id, name, stripe_account_id, stripe_connect_status, surcharge_enabled, surcharge_percentage, tax_enabled, tax_percentage)')
          .eq('user_id', user.id)
          .eq('active', true)
          .single()
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8">
            ...
            <div className="bg-[#F8F9FC] rounded-xl border border-[#DDE1EC] p-5 mb-4">
              ...
              <p className="text-xs text-[#6B7490] mt-2">
                ⚠️ Surcharge laws vary by state. Confirm this is permitted in your state before enabling.
              </p>
            </div>
            ...
            <div className="bg-[#F8F9FC] rounded-xl border border-[#DDE1EC] p-5 mb-6">
              ...
              <p className="text-xs text-[#6B7490] mt-2">
                🏛️ NH, FL, and TX have no sales tax on most services. Check your state's rules.
              </p>
            </div>
            ...
            <button
              onClick={() => { setShowSetup(false); router.replace('/dashboard/settings/stripe-connect') }}
              ...
              {surchargeEnabled && (
                <div className="flex items-center gap-2 mt-2">
                  <input
                    ...
                    value={surchargePercentage}
                    onChange={e => setSurchargePercentage(e.target.value)}
                    className="w-20 bg-[#F8F9FC] border border-[#DDE1EC] rounded-lg px-3 py-1.5 text-sm text-[#0E1117] focus:outline-none focus:border-[#4F8EF7]"
                  />
                ...
                {payments.map(p => (
                  <div key={p.id} className="px-4 py-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-[#6B7490]">
                        {p.completed_at ? new Date(p.completed_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}
                      </span>
                    </div>
                  </div>
                ))}
