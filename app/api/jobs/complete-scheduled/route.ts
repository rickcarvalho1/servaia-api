...
async function sendSMSReceipt({ phone, customerName, businessName, serviceNames, totalDollars }: any) {
  const twilioSid = process.env.TWILIO_ACCOUNT_SID
  const twilioAuth = process.env.TWILIO_AUTH_TOKEN
  const twilioPhone = process.env.TWILIO_PHONE_NUMBER
  if (!twilioSid || !twilioAuth || !twilioPhone) return false

  const body = `Hi ${customerName}, your service from ${businessName} is complete. Services: ${serviceNames}. Total charged: $${totalDollars}. Thank you!`
...
async function sendEmailReceipt({ email, customerName, businessName, services, totalDollars }: any) {
...
export async function POST(request: Request) {
  try {
    ...
    if (customer.phone) {
      smsSent = await sendSMSReceipt({ phone: customer.phone, customerName, businessName, serviceNames, totalDollars })
    }
    if (customer.email) {
      emailSent = await sendEmailReceipt({ email: customer.email, customerName, businessName, services, totalDollars })
    }
